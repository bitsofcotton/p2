...
# General Tips 3
With bitsofcotton/randtools, any fixed operations on N is described as fixed A in N^{n\*n} with the form y := A * [0, 1, x]. So with such numerical series, with x_n+1 - x_n pairs, x_nexts := A * [0, 1, x_nows], with x_nows = [x_0, ..., x_now], x_now = [x_now_0, ..., x_now_m], X_now := [x_now_0 + x_now_1 \* 2 + ... + x_now_m * 2^m] 
