...
# Uncertain Tips:
With beating with normal PRNGs by beat.py, the result is flat 0 delta line with ideal prediction function. So with this, non one-function-recursion results can be tested by such beat.py with prediction. If the result isn't flat enough, it might mean the original series isn't consisted by such function.
