...
However, some of the numerical calculation suggests if this p.cc:p arg fails, p0.cc:p0 -arg succeeds. But we cannot find the clue why the PRNGs and the data we have on the PC connected SSD behaves the same as them (But they suggests highly probabilities by infection, even the counting method we have also.).
