...
#define int int64_t
  ...
  PBond<num_t, Prange<num_t, deltaFeeder<num_t, idFeeder<num_t> >, deltaFeeder<num_t, cbrtFeeder<num_t, idFeeder<num_t> > > >, sumFeeder<num_t, idFeeder<num_t> >, invFeeder<num_t, sumFeeder<num_t, idFeeder<num_t> > > > p(Prange<num_t, deltaFeeder<num_t, idFeeder<num_t> >, deltaFeeder<num_t, cbrtFeeder<num_t, idFeeder<num_t> > > >(abs(status)), 1 + 3 + int(ceil(num_t(abs(status) - 1) / (pow(num_t(abs(status) + 1), num_t(int(2)) / num_t(int(3)) ) - pow(num_t(abs(status)), num_t(int(2)) / num_t(int(3)) ) )) ) );
  PBond<num_t, Prange<num_t, deltaFeeder<num_t, idFeeder<num_t> >, deltaFeeder<num_t, idFeeder<num_t> > >, sumFeeder<num_t, idFeeder<num_t> >, invFeeder<num_t, sumFeeder<num_t, idFeeder<num_t> > > > q(Prange<num_t, deltaFeeder<num_t, idFeeder<num_t> >, deltaFeeder<num_t, idFeeder<num_t> > >(abs(status)), abs(status) + 1);
  num_t d(int(0));
  auto  M(d);
