...
# endif
  ...
  case 'r': case 'R': {
    ...
    while(std::getline(std::cin, s, '\n')) {
      b.next(s2sv<num_t>(s));
      if(! b.full) continue;
      SimpleVector<SimpleVector<num_t> > p(pPRNG3<num_t, 0>(offsetHalf<num_t>(b.res), 8, string("")));
      for(int i = 0; i < p.size() - 1; i ++) {
        for(int j = 0; j < p[i].size(); j ++) {
          const num_t& ref(b.res[i - (p.size() - 1) + b.res.size()][j]);
          std::cout << (argv[1][1] == '\0' ? p[i][j] : ref - (p[i][j] /= sgn<num_t>(ref))) << ", ";
        }
...
# endif
    ...
    while(std::getline(std::cin, s, '\n')) {
      ...
      case 'T': {
        ...
        const int i(in.size() / 2 - 1);
        std::cout << (in[i] + in[i + in.size() / 2]) * (argv[1][1] == '+' ?
          num_t(int(1)) : in[i + in.size() / 2]) << std::endl;
        break;
      } case 'J': {
